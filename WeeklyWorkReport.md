...
#### 12-07 14:38
1. 升级分桶查询，召回诊断的channel获取，判定等逻辑，升级召回诊断编辑框的样式。
2. 完成告警移动端前端页面开发，更新codemirror设置，缩小为一个齿轮。
3. 系统日常维护。

#### 11-20 11:50
1. 告警服务使用前一分钟的数据进行计算。
2. Rrank发布历史中，修复前端按钮disable逻辑和后端获取预发布/发布bug。
3. 更新Policy.proto，对齐和play交互的字段。

##### 11-05 16:00
1. 完成Rank模块前后端对多责任人的支持。
2. 重构Rank模块的前端发布逻辑，包括变量控制和统一改为异步请求等。
...
- [x] TriggerDebug中HG单独支持omgid和hgid。
- [x] 增删改的时候，要同步更新cite_map的信息，可以用LIKE进行模糊匹配。
- [x] Rerank脚本前端侧，谁都可以添加/编辑公共脚本，但是只有global以上用户可以上线。
