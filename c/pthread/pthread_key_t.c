...
void *child1(void *arg) {
  ...
  printf(
      "child1--from pthread_getspecific(key) get the pointer and it points to "
      "--> 0x%p\n",
      (struct test_struct *)pthread_getspecific(key));
  printf(
      "child1--from pthread_getspecific(key) get the pointer and print it's "
      "content:\nstruct_data.i:%d\nstruct_data.k: %f\n",
...
void *child2(void *arg) {
  ...
  printf(
      "child2--from pthread_getspecific(key) get the pointer and it points to "
      "--> 0x%p\n",
      (int *)pthread_getspecific(key));
  printf(
      "child2--from pthread_getspecific(key) get the pointer and print it's "
      "content --> temp:%d\n",
      *((int *)pthread_getspecific(key)));
}
int main(void) {
  pthread_t tid1, tid2;
  // 这里是构建一个pthread_key_t类型，确实是相当于一个key
